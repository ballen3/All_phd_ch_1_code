...
# output
output_dir="$HOME/slu_raw/scaffolds/scaff_quast_output"
...
#load quast from maple conda env- actually quast version 5.2.0, the env name is weird
source activate quast-4.6.3

# Create a directory for the sample within the output directory
mkdir -p {output_dir}/{prefix}_output

# Run quast
echo "Running QUAST for {prefix}"
quast.py -o {output_dir}/{prefix}_output --threads 8 -e --fungus -g "{gff}" --fragmented -r "{ref}" -s $HOME/slu_raw/deinterleaved_slu/scaffolds/{file_name}

cd $HOME/slu_raw/scaffolds/scaff_quast_output/{prefix}_output || exit

# rename outputs with filenames
mv contigs_reports {prefix}_contigs_reports
mv report.txt {prefix}_report.txt
mv report.html {prefix}_report.html
mv transposed_report.txt {prefix}_transposed_report.txt
mv transposed_report.tsv {prefix}_transposed_report.tsv
mv icarus_viewers {prefix}_icarus_viewers
mv genome_stats {prefix}_genome_stats
mv aligned_stats {prefix}_aligned_stats
mv basic_stats {prefix}_basic_stats
mv icarus.html {prefix}_icarus.html
mv report.pdf {prefix}_report.pdf
mv report.tsv {prefix}_report.tsv


echo "QUAST for {prefix} done!"
"""

    # Generate script file name
    script_filename = f"quast_{prefix}.sh"
